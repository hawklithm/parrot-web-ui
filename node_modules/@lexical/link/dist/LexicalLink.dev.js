...
class LinkNode extends lexical.ElementNode {
  ...
  createDOM(config) {
    ...
    lexical.addClassNamesToElement(element, config.theme.link);
    return element;
  }
  updateLinkDOM(prevNode, anchor, config) {
    if (lexical.isHTMLAnchorElement(anchor)) {
...
function $linkNodeTransform(link) {
  ...
  for (const caret of lexical.$getChildCaret(link, 'next')) {
    ...
    if (lexical.$isElementNode(node) && !node.isInline()) {
      ...
      lexical.$insertNodeToNearestRootAtCaret(node, lexical.$rewindSiblingCaret(caret), {
        $shouldSplit: () => false
      });
    }
  }
...
function $convertAnchorElement(domNode) {
  let node = null;
  if (lexical.isHTMLAnchorElement(domNode)) {
...
const TOGGLE_LINK_COMMAND = /* @__PURE__ */lexical.createCommand('TOGGLE_LINK_COMMAND');
...
function $toggleLink(urlOrAttributes, attributes = {}) {
  ...
  if (lexical.$isNodeSelection(selection)) {
    ...
    nodes.forEach(node => {
      if (url === null) {
        // Remove link
        const linkParent = lexical.$findMatchingParent(node, parent => !$isAutoLinkNode(parent) && $isLinkNode(parent));
        if (linkParent) {
        ...
        // Add/Update link
        const existingLink = lexical.$findMatchingParent(node, parent => !$isAutoLinkNode(parent) && $isLinkNode(parent));
        if (existingLink) {
  ...
  if (selection.isCollapsed() && url === null) {
    for (const node of selection.getNodes()) {
      const parentLink = lexical.$findMatchingParent(node, parent => !$isAutoLinkNode(parent) && $isLinkNode(parent));
      if (parentLink !== null) {
        parentLink.getParentOrThrow().splice(parentLink.getIndexWithinParent(), 0, parentLink.getChildren());
        parentLink.remove();
      }
      return;
    }
  ...
  if (nodes.length === 1) {
    ...
    const linkNode = lexical.$findMatchingParent(firstNode, $isLinkNode);
    if (linkNode !== null) {
  ...
  $withSelectedNodes(() => {
    ...
    for (const node of nodes) {
      ...
      const parentLinkNode = lexical.$findMatchingParent(node, $isLinkNode);
      if (parentLinkNode) {
...
const AnchorRule = /* @__PURE__ */html.defineImportRule({
  $import: (ctx, el) => {
...
function registerLink(editor, stores) {
  return lexical.mergeRegister(editor.registerNodeTransform(LinkNode, $linkNodeTransform), editor.registerCommand(TOGGLE_LINK_COMMAND, payload => {
 ...
 */
const LinkExtension = /* @__PURE__ */lexical.defineExtension({
 ...
 * it directly instead.
 */
const LinkImportExtension = /* @__PURE__ */lexical.defineExtension({
...
function registerClickableLink(editor, stores, eventOptions = {}) {
  const onClick = event => {
    ...
    nearestEditor.update(() => {
      ...
      if (clickedNode !== null) {
        const maybeLinkNode = lexical.$findMatchingParent(clickedNode, lexical.$isElementNode);
        if (!stores.disabled.peek()) {
          if ($isLinkNode(maybeLinkNode)) {
            url = maybeLinkNode.sanitizeUrl(maybeLinkNode.getURL());
            urlTarget = maybeLinkNode.getTarget();
          } else {
            const a = findMatchingDOM(target, lexical.isHTMLAnchorElement);
            if (a !== null) {
              url = a.href;
              urlTarget = a.target;
            }
    ...
    const selection = editor.read('latest', lexical.$getSelection);
 ...
 */
const ClickableLinkExtension = /* @__PURE__ */lexical.defineExtension({
  build(editor, config, state) {
    return extension.namedSignals(config);
  },
...
function registerAutoLink(editor, config = defaultConfig) {
  ...
  return lexical.mergeRegister(editor.registerNodeTransform(lexical.TextNode, textNode => {
 ...
 */
const AutoLinkExtension = /* @__PURE__ */lexical.defineExtension({
