...
import type { CreateConfigValues } from "../lib/paperclip-adapter-utils/src";

// Re-export shared types so local consumers don't need to change imports
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "../lib/paperclip-adapter-utils/src";

export interface StatefulStdoutParser {
  parseLine: (line: string, ts: string) => import("parrot-adapter-utils").TranscriptEntry[];
  reset: () => void;
}
...
export interface TranscriptParserSource {
  parseStdoutLine: (line: string, ts: string) => import("parrot-adapter-utils").TranscriptEntry[];
