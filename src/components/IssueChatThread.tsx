      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Choose whether voted AI outputs can be shared with Parrot Labs. This
...
function SystemNoticeCommentRow({
  ...
  const source = (() => {
    ...
    if (authorType === "system") {
      const label = runAgentName ?? "Parrot";
      if (runAgentId && runId) return { label, href: `/agents/${runAgentId}/runs/${runId}` };
      return { label };
    }
    if (runAgentId && runId) {
      return { label: authorName ?? runAgentName ?? "Parrot", href: `/agents/${runAgentId}/runs/${runId}` };
    }
