...
import type { AuthSession } from "../lib/paperclip-shared/src";
  ...
  return (
    <div className="mt-6 border-t border-border pt-5">
      ...
      <p className="mt-2 text-sm text-muted-foreground">
        {hasActiveInvite
          ? "A bootstrap invite is already active. Check your Parrot startup logs for the first-admin URL, or run this command on the host to rotate it:"
          : "Run this command on the host that runs Parrot to print a one-time first-admin invite URL:"}
      </p>
...
export function BootstrapPendingPage({
  ...
  if (!claimAvailable) {
    return (
      <StateChrome>
        <h1 className="text-xl font-semibold">This Parrot is waiting on its first admin</h1>
        <p className="mt-2 text-sm text-muted-foreground">
  ...
  if (!session) {
    return (
      <StateChrome>
        <h1 className="text-xl font-semibold">Finish setting up this Parrot</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          No admin has claimed this instance yet. Sign in or create your Parrot account to become the first
