...
describe("Secrets page layout", () => {
  ...
  it("warns that removing a provider vault only removes Parrot config", async () => {
    ...
    expect(document.body.textContent).toContain("from Parrot only");
    ...
    const confirmButton = [...document.querySelectorAll("button")].find(
      (button) => button.textContent?.includes("Remove from Parrot"),
    ) as HTMLButtonElement | undefined;
    await act(async () => {
      confirmButton?.click();
    });
