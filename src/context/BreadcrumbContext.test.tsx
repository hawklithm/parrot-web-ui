...
describe("BreadcrumbContext", () => {
  ...
  it("builds page titles with the selected company name before Parrot", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "Anachronist Wiki")).toBe(
      "Inbox • Anachronist Wiki • Parrot",
    );
    expect(
      buildDocumentTitle(
    ...
    ).toBe("PAP-3515 • Issues • Anachronist Wiki • Parrot");
  });

  it("omits blank company names from page titles", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "  ")).toBe("Inbox • Parrot");
    expect(buildDocumentTitle([], null)).toBe("Parrot");
  });
});
