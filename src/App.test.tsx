...
describe("CloudAccessGate", () => {
  ...
  it("shows browser sign-in setup for signed-out private bootstrap-pending instances", async () => {
    ...
    const root = renderGate(container);
    await waitForText(container, "Finish setting up this Parrot");

    expect(container.textContent).toContain("Finish setting up this Parrot");
  ...
  it("keeps public bootstrap-pending instances invite-only", async () => {
    ...
    const root = renderGate(container);
    await waitForText(container, "This Parrot is waiting on its first admin");

    expect(container.textContent).toContain("This Parrot is waiting on its first admin");
